...
const xvfbLog = "/tmp/claude-rehost/xvfb.log";
...
const xvfbPidFile = "/tmp/claude-rehost/xvfb.pid";
...
function summarizeProcessTable() {
  return run("ps", ["-ef"]).split("\n").filter((line) => {
    ...
    return /(electron|xstartup|xvfb|health-server|node)/.test(lower);
  }).join("\n");
}
  ...
  .createServer((_req, res) => {
    ...
    const body = {
      ...
      pids: {
        ...
        xvfb: Number(readText(xvfbPidFile).trim()) || null,
      },
